...
(select
    ...
    ,count( if(loans_by_cli > 0, c_id, NULL) ) as "W/ Loan"
	,count(if(loans_by_cli = 0, c_id, NULL) ) as "W/o Loan"
	,sum(loans_by_cli) as "Loans"
...
from(
	select
		o_s.branch as branch_by_cli 
		,o_s.mentor as mentor_by_cli
		,o_s.center as center_by_cli  
		,group_mtg_day.mtg_day as mtg_day_by_cli 
		,c.id as c_id
		,count(l.id) as loans_by_cli 
