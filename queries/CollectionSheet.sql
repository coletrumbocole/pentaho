...
,sum(L.ln_amount) as LnAmount
,'' as "Ln Type"
,sum(L.ln_bal) as "Ln Bal"
,sum(L.total_to_close) as "Total to Close"
,'' as "Term"
,'' as "RW"
,sum(L.prin_cd) as "Prin (Current Due)"
,sum(L.int_cd) as "Int (Current Due)"
,'' as "Delay"
,sum(L.prin_od) as "Prin (Overdue)"
,sum(L.int_od) as "Int  (Overdue)"
,sum(L.ln_pmts) as "Ln Pmts"
,sum(C.cf) as "CF (Weekly Due)"
,sum(C.gf) as "GF (Weekly Due)"
,sum(L.fee) as "Fee/ Dayong"
,sum( ifnull(L.ln_pmts, 0) + ifnull(C.cf, 0) + ifnull(C.gf, 0) + ifnull(L.fee, 0) ) as "Total Due"
,sum(C.ps_bal) as "Ps Bal"
