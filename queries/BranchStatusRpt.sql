...
from(
	select
		...
		# ,count(if(l.id = NULL, c.id, NULL) ) as w_o_loan
	...
	# New loans: How many loans were disbursed this week?
		...
		# ,sum(arrears amt) / sum(outbal) as "DR %"
		# sum (par amt) / sum (outbal). "PAR %"
		,'' as age_by_cli
		#,sum(if(laa.loan_id <> NULL, l.principal_outstanding_derived, 0)) as "PAR Amt 2" #why 2nd time?
		#,sum(if(laa.loan_id <> NULL, l.principal_outstanding_derived, 0)) / sum(l.principal_outstanding_derived) as "PAR % 2" #why 2nd time?
